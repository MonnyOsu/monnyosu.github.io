...
import taskFactory, { getDueDateLengthDays } from './task.js';
...
deleteCompleteTasks.addEventListener('click', (e) => {
  ...
  if (selectedProjectId === 'today-id' || selectedProjectId === 'this-week-id') {
    ...
    projects.forEach((project) => {
      project.tasks = project.tasks.filter((task) => !completedTasks.includes(task));
    });
    saveAndRender();
  } else {
...
sortButton.addEventListener('click', (e) => {
  const selectedProject = getSelectedProject(selectedProjectId);
  selectedProject.tasks = selectedProject.tasks.sort((a, b) => {
...
function updateTodayTasks() {
  ...
  defaultProjects[0].tasks = defaultProjects[0].tasks.sort((a, b) => {
    if (a.dueDate < b.dueDate) {
      return -1;
    }
    if (a.dueDate > b.dueDate) {
...
function updateThisWeekTasks() {
  ...
  defaultProjects[1].tasks = defaultProjects[1].tasks.sort((a, b) => {
    if (a.dueDate < b.dueDate) {
      return -1;
    }
    if (a.dueDate > b.dueDate) {
